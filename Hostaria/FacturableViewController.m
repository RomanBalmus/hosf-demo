...
#import "Country.h"
@interface FacturableViewController ()
{
...
@implementation FacturableViewController
...
- (void)viewDidLoad {
    ...
    factureData=[[NSMutableDictionary alloc]init];
    isSelectedSwitch=NO;
    ...
    if ([self dictionary:user hasKey:@"email"] ) {
        [factureData setObject:[user objectForKey:@"email"] forKey:@"email"];
        ...
        if ([v isKindOfClass:[UITextField class]]){
            ...
            if (tf.tag==41) {
                if ([self dictionary:user hasKey:@"name"] ) {
                    ...
                    [factureData setObject:[user objectForKey:@"name"] forKey:@"name"];
                    
                    [tf setText:[factureData objectForKey:@"name"]];
                }
            ...
            if (tf.tag==42) {
                ...
                if ([self dictionary:user hasKey:@"surname"] ) {
                    [factureData setObject:[user objectForKey:@"surname"] forKey:@"surname"];

                    [tf setText:[factureData objectForKey:@"surname"]];
                }
            ...
            if (tf.tag==43) {
                [factureData setObject:tf.text forKey:@"dayPhone"];
                
            }
            if (tf.tag==44) {
                [factureData setObject:tf.text forKey:@"eveningPhone"];
                
            } if (tf.tag==45) {
                [factureData setObject:tf.text forKey:@"cellPhone"];
                
            } if (tf.tag==46) {
                [factureData setObject:tf.text forKey:@"countryDisplayName"];
                [factureData setObject:tf.text forKey:@"countryIso"];
            } if (tf.tag==47) {
                [factureData setObject:tf.text forKey:@"addressLine1"];
                
            }  if (tf.tag==48) {
                [factureData setObject:tf.text forKey:@"addressLine2"];
                
            } if (tf.tag==49) {
                [factureData setObject:tf.text forKey:@"city"];
                
            } if (tf.tag==50) {
                [factureData setObject:tf.text forKey:@"province"];
                
            } if (tf.tag==51) {
                [factureData setObject:tf.text forKey:@"zipCode"];
                
            } if (tf.tag==52) {
                [factureData setObject:tf.text forKey:@"businessName"];
                
            }if (tf.tag==53) {
                [factureData setObject:tf.text forKey:@"fiscalCode"];
                
            }if (tf.tag==54) {
                [factureData setObject:tf.text forKey:@"vatNumber"];
    ...
    NSLog(@"facture data init %@",factureData);
...
- (void)doneTouched:(UIBarButtonItem *)sender{
    ...
    [factureData setObject:country.countryIso forKey:@"countryIso"];
    [factureData setObject:country.countryDisplayName forKey:@"countryDisplayName"];
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    switch (textField.tag) {
        case 41:
            [factureData setObject:textField.text forKey:@"name"];
            break;
        case 42:
            [factureData setObject:textField.text forKey:@"surname"];
            break;
        case 43:
            [factureData setObject:textField.text forKey:@"dayPhone"];
            break;
        case 44:
            [factureData setObject:textField.text forKey:@"eveningPhone"];
            break;
        case 45:
            [factureData setObject:textField.text forKey:@"cellPhone"];
            break;
        case 47:
            [factureData setObject:textField.text forKey:@"addressLine1"];
            break;
        case 48:
            [factureData setObject:textField.text forKey:@"addressLine2"];
            break;
        case 49:
            [factureData setObject:textField.text forKey:@"city"];
            break;
        case 50:
            [factureData setObject:textField.text forKey:@"province"];
            break;
        case 51:
            [factureData setObject:textField.text forKey:@"zipCode"];
            break;
        case 52:
            [factureData setObject:textField.text forKey:@"businessName"];
            break;
        case 53:
            [factureData setObject:textField.text forKey:@"fiscalCode"];
            break;
        case 54:
            [factureData setObject:textField.text forKey:@"vatNumber"];
            break;
        default:
            break;
    }
...
- (IBAction)nextButtonCliked:(id)sender {
    ...
    NSLog(@"start to store data %@ data is good: %d",factureData,isGoodData);
    ...
    if (isGoodData) {
        ...
        if (isSelectedSwitch) {
            if ([self.delegate respondsToSelector:@selector(bothData:)]) {
                [self.delegate bothData:factureData];
            }
            
            
          
        }else{
            if ([self.delegate respondsToSelector:@selector(factureData:)]) {
                [self.delegate factureData:factureData];
            }
